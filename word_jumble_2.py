...
# Python 3 is required, so run with this command:  python3 word_jumble.py
...
def solve_one_jumble(letters):
    ...
    filtered_by_len = list(filter(lambda word: len(word) == letters_len, words_list))
    # check that letters
    main_str_hist = histogram_generator(letters)
